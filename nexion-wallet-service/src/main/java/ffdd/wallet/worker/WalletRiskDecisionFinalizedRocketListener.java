...
public class WalletRiskDecisionFinalizedRocketListener {
    private static final Logger log = LoggerFactory.getLogger(WalletRiskDecisionFinalizedRocketListener.class);
    private static final String EVENT_RISK_DECISION_FINALIZED = "RiskDecisionFinalized";
    ...
    public WalletRiskDecisionFinalizedRocketListener(
    ...
    public void start() throws Exception {
        ...
        consumer.subscribe(topic, EVENT_RISK_DECISION_FINALIZED);
        ...
        consumer.start();
        log.info("Wallet risk decision RocketMQ listener started topic={}, group={}, nameServer={}, maxReconsumeTimes={}",
    ...
    boolean consumeMessage(MessageExt rocketMessage) {
        ...
        if (!claim.claimed()) {
            log.info("Wallet risk decision listener skipped duplicate eventId={}, status={}, attempts={}, msgId={}",
                    claim.eventId(), claim.status(), claim.attemptCount(), rocketMessage.getMsgId());
            return false;
        }
        if (!EVENT_RISK_DECISION_FINALIZED.equals(message.getEventType())) {
            deliveryService.markSkipped(
                    consumerGroup, claim.eventId(), "Unsupported event type " + message.getEventType());
            log.info("Wallet risk decision listener skipped eventType={}, eventId={}, msgId={}",
                    message.getEventType(), claim.eventId(), rocketMessage.getMsgId());
            return false;
        }

        try {
            RiskDecisionFinalizedPayload payload =
                    objectMapper.readValue(message.getPayload(), RiskDecisionFinalizedPayload.class);
            RiskDecisionApplyResult result = walletService.applyRiskDecision(toRequest(payload));
            deliveryService.markSuccess(consumerGroup, claim.eventId(), result == null ? 0 : 1);
            log.info("Wallet risk decision listener applied decisionNo={}, bizType={}, bizNo={}, status={}, eventId={}, msgId={}, reconsumeTimes={}",
                    payload.getDecisionNo(), payload.getBizType(), payload.getBizNo(),
                    result == null ? null : result.getStatus(), claim.eventId(),
    ...
    private ApplyRiskDecisionRequest toRequest(RiskDecisionFinalizedPayload payload) {
        ApplyRiskDecisionRequest request = new ApplyRiskDecisionRequest();
        request.setDecisionId(payload.getDecisionId());
        request.setDecisionNo(payload.getDecisionNo());
        request.setBizType(payload.getBizType());
        request.setBizNo(payload.getBizNo());
        request.setDecision(payload.getDecision());
    ...
    private boolean recordMalformedFailure(MessageExt rocketMessage, Exception ex) {
        try {
            ...
            if (!claim.claimed()) {
                log.info("Wallet risk decision listener skipped malformed duplicate eventId={}, status={}, msgId={}",
    ...
    private boolean markFailure(String eventId, MessageExt rocketMessage, Exception ex) {
        try {
            ...
            if (failure.dead()) {
                log.warn("Wallet risk decision listener moved poison delivery to DEAD eventId={}, msgId={}, attempts={}, reconsumeTimes={}, error={}",
                        failure.eventId(), rocketMessage.getMsgId(), failure.attemptCount(),
                        rocketMessage.getReconsumeTimes(), errorMessage(ex));
                return false;
            }
            log.warn("Wallet risk decision listener will retry eventId={}, msgId={}, attempts={}, reconsumeTimes={}, error={}",
                    failure.eventId(), rocketMessage.getMsgId(), failure.attemptCount(),
                    rocketMessage.getReconsumeTimes(), errorMessage(ex));
            return true;
        } catch (RuntimeException deliveryEx) {
    ...
    private boolean fallbackRetry(MessageExt rocketMessage, RuntimeException ex) {
        ...
        log.warn("Wallet risk decision listener delivery-state error msgId={}, reconsumeTimes={}, retry={}, error={}",
