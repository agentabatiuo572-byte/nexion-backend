    ...
    private boolean consumeMessage(MessageExt rocketMessage) {
        try {
            ...
            if (!EVENT_EARNING_GENERATED.equals(message.getEventType())) {
                log.info("Wallet RocketMQ listener skipped eventType={}, msgId={}",
            ...
            log.info("Wallet RocketMQ listener posted earning eventNo={}, ledgerId={}, eventId={}, msgId={}, reconsumeTimes={}",
                    payload.getEventNo(), ledger == null ? null : ledger.getId(), message.getEventId(),
                    rocketMessage.getMsgId(), rocketMessage.getReconsumeTimes());
            return false;
        } catch (Exception ex) {
            if (rocketMessage.getReconsumeTimes() >= maxReconsumeTimes) {
                log.warn("Wallet RocketMQ listener dropped poison msgId={}, reconsumeTimes={}, error={}",
                        rocketMessage.getMsgId(), rocketMessage.getReconsumeTimes(), ex.getMessage());
                return false;
            }
            log.warn("Wallet RocketMQ listener will retry msgId={}, reconsumeTimes={}, error={}",
                    rocketMessage.getMsgId(), rocketMessage.getReconsumeTimes(), ex.getMessage());
            return true;
        }
    }
