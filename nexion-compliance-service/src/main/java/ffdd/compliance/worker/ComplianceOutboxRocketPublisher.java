...
public class ComplianceOutboxRocketPublisher {
    public static final String EVENT_RISK_DECISION_FINALIZED = "RiskDecisionFinalized";
    private static final Logger log = LoggerFactory.getLogger(ComplianceOutboxRocketPublisher.class);
    ...
    public ComplianceOutboxRocketPublisher(
            EventOutboxService outboxService,
            DefaultMQProducer complianceOutboxRocketProducer,
        ...
        this.producer = complianceOutboxRocketProducer;
    ...
    public void publishScheduled() {
        try {
            ...
            if (result.getScanned() > 0 || result.getFailed() > 0) {
                log.info("Compliance outbox RocketMQ publisher scanned={}, published={}, skipped={}, failed={}",
                        result.getScanned(), result.getPublished(), result.getSkipped(), result.getFailed());
            }
        } catch (RuntimeException ex) {
            log.warn("Compliance outbox RocketMQ publisher failed: {}", ex.getMessage());
        }
    }
    ...
    public OutboxPublishResult publishPending(int limit) {
        ...
        List<EventOutboxMessage> messages = outboxService.listPendingByEventType(
                EVENT_RISK_DECISION_FINALIZED, normalizedLimit);
