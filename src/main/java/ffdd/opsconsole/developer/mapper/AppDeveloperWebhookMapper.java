...
public interface AppDeveloperWebhookMapper extends BaseMapper<Object> {
    ...
    @Select("SELECT id FROM nx_developer_webhook WHERE user_id=#{userId} AND source_environment=#{sourceEnvironment} AND run_id=#{runId} AND status='ACTIVE' AND is_deleted=0 AND JSON_CONTAINS(events_json, JSON_QUOTE(#{eventType}))")
    ...
    int rotateSecret(@Param("id") Long id, @Param("userId") Long userId, @Param("sourceEnvironment") String sourceEnvironment, @Param("runId") String runId, @Param("version") Long version, @Param("secretHash") String secretHash, @Param("rotationKey") String rotationKey, @Param("rotationHash") String rotationHash);
    @Update("UPDATE nx_developer_webhook SET secret_hash=#{secretHash},secret_ciphertext=#{secretCiphertext},secret_rotation_key=#{rotationKey},secret_rotation_hash=#{rotationHash},version=version+1,updated_at=NOW() WHERE id=#{id} AND user_id=#{userId} AND source_environment=#{sourceEnvironment} AND run_id=#{runId} AND version=#{version} AND is_deleted=0")
