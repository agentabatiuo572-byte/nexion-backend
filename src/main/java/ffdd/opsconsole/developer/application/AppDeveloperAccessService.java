...
public class AppDeveloperAccessService {
    ...
    private Scope scope(Long userId, boolean lock) {
        ...
        boolean isolated = active.size() == 1 && java.util.Set.of("test").contains(active.iterator().next());
        boolean production = active.isEmpty() || (active.size() == 1 && java.util.Set.of("dev", "prod").contains(active.iterator().next()));
