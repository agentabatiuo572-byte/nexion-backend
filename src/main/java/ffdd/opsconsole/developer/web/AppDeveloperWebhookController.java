...
public class AppDeveloperWebhookController {
    ...
    @DeleteMapping("/{id}") public ApiResult<Void> delete(@PathVariable Long id, Authentication auth) { Long uid = userId(auth); return uid == null ? ApiResult.fail(403, "USER_AUTH_REQUIRED") : service.delete(uid, id); }
    @PostMapping("/{id}/enable") public ApiResult<Map<String,Object>> enable(@PathVariable Long id, Authentication auth) { Long uid = userId(auth); return uid == null ? ApiResult.fail(403, "USER_AUTH_REQUIRED") : service.setEnabled(uid, id, true); }
    @PostMapping("/{id}/disable") public ApiResult<Map<String,Object>> disable(@PathVariable Long id, Authentication auth) { Long uid = userId(auth); return uid == null ? ApiResult.fail(403, "USER_AUTH_REQUIRED") : service.setEnabled(uid, id, false); }
    @GetMapping("/{id}/deliveries") public ApiResult<List<Map<String,Object>>> deliveries(@PathVariable Long id, @RequestParam(defaultValue = "50") int limit, Authentication auth) { Long uid = userId(auth); return uid == null ? ApiResult.fail(403, "USER_AUTH_REQUIRED") : service.deliveryLog(uid, id, limit); }
