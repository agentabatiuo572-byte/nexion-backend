...
public class AppDeveloperApiController {
    ...
    @DeleteMapping("/{id}") public ApiResult<Map<String,Object>> revoke(@PathVariable Long id, @RequestHeader(value = "Idempotency-Key", required = false) String key, Authentication auth) { Long uid = userId(auth); return uid == null ? ApiResult.fail(403, "USER_AUTH_REQUIRED") : service.revoke(uid, id, key); }
    @PostMapping("/{id}/revoke") public ApiResult<Map<String,Object>> revokePost(@PathVariable Long id, @RequestHeader(value = "Idempotency-Key", required = false) String key, Authentication auth) { Long uid = userId(auth); return uid == null ? ApiResult.fail(403, "USER_AUTH_REQUIRED") : service.revoke(uid, id, key); }
