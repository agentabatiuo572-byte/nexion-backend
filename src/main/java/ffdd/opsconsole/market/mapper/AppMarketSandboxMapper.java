...
/**
 * MySQL-only boundary for the explicit App market sandbox.  None of these
 * statements touch a production wallet, order, holding or ledger table.
 */
@Mapper
...
public interface AppMarketSandboxMapper {
    ...
    @Select("SELECT COUNT(*) FROM nx_genesis_sandbox_holding WHERE run_id=#{runId}")
    ...
    @Select("SELECT biz_no AS bizNo,direction,amount,balance_after AS balanceAfter,remark,created_at AS createdAt FROM nx_genesis_sandbox_ledger WHERE run_id=#{runId} AND user_id=#{userId} ORDER BY id DESC LIMIT 100")
