...
public interface AppExchangeMapper {
    ...
    @Select("""
            WITH base AS (
                SELECT id,price_usdt AS priceUsdt,sampled_at AS sampledAt
                 FROM nx_price_index
                 WHERE metric_code IN ('NEX','NEX_USDT') AND status='ACTIVE' AND is_deleted=0
                   AND sampled_at>=DATE_SUB(NOW(),INTERVAL 366 DAY)
                   AND sampled_at<=DATE_ADD(NOW(),INTERVAL 1 MINUTE)
            ), fiveMinute AS (
                SELECT MAX(id) AS id FROM base WHERE sampledAt>DATE_SUB(NOW(),INTERVAL 24 HOUR)
                 GROUP BY DATE(sampledAt),HOUR(sampledAt),FLOOR(MINUTE(sampledAt)/5)
            ), hourly AS (
                SELECT MAX(id) AS id FROM base WHERE sampledAt>DATE_SUB(NOW(),INTERVAL 7 DAY)
                 GROUP BY DATE(sampledAt),HOUR(sampledAt)
            ), daily AS (
                SELECT MAX(id) AS id FROM base WHERE sampledAt>DATE_SUB(NOW(),INTERVAL 30 DAY)
                 GROUP BY DATE(sampledAt)
            ), monthly AS (
                SELECT MAX(id) AS id FROM base GROUP BY YEAR(sampledAt),MONTH(sampledAt)
            ), boundaries AS (
                SELECT MAX(id) AS id FROM base WHERE sampledAt<=DATE_SUB(NOW(),INTERVAL 1 HOUR)
                UNION SELECT MAX(id) AS id FROM base WHERE sampledAt<=DATE_SUB(NOW(),INTERVAL 24 HOUR)
                UNION SELECT MAX(id) AS id FROM base WHERE sampledAt<=DATE_SUB(NOW(),INTERVAL 7 DAY)
                UNION SELECT MAX(id) AS id FROM base WHERE sampledAt<=DATE_SUB(NOW(),INTERVAL 30 DAY)
                UNION SELECT MAX(id) AS id FROM base WHERE sampledAt<=DATE_SUB(NOW(),INTERVAL 365 DAY)
            ), selected AS (
                SELECT id FROM fiveMinute UNION SELECT id FROM hourly UNION SELECT id FROM daily
                UNION SELECT id FROM monthly UNION SELECT id FROM boundaries
            )
            SELECT b.priceUsdt,b.sampledAt
              FROM base b JOIN selected s ON s.id=b.id
             WHERE s.id IS NOT NULL
             ORDER BY b.sampledAt,b.id
            """)
    List<MarketPoint> marketHistoryPoints();
    ...
    @Select("""
            SELECT COUNT(1) FROM nx_wallet_ledger
             WHERE biz_no=CONCAT(#{exchangeNo},'-HOLD') AND biz_type='EXCHANGE_SWAP'
               AND direction='OUT' AND status='SUCCESS' AND is_deleted=0
            """)
    ...
    @Select("""
            SELECT user_id AS userId,exchange_no AS exchangeNo,UPPER(from_asset) AS fromAsset,from_amount AS fromAmount
             ...
             LIMIT 1 FOR UPDATE
            """)
    QueuedRow lockOwnQueued(@Param("userId") Long userId,@Param("exchangeNo") String exchangeNo);
    ...
    @Update("""
            UPDATE nx_exchange_order SET status='FAILED',updated_at=NOW()
             WHERE exchange_no=#{exchangeNo} AND UPPER(status)='QUEUED' AND is_deleted=0
               AND EXISTS (SELECT 1 FROM nx_user u WHERE u.id=nx_exchange_order.user_id
                           AND u.status='ACTIVE' AND u.is_deleted=0 AND COALESCE(u.sandbox,0)=0)
            """)
    int failQueuedInsufficient(@Param("exchangeNo") String exchangeNo);
