...
public interface SupportTicketMapper extends BaseMapper<SupportTicketEntity> {
    @Select("SELECT COUNT(*) FROM nx_support_ticket WHERE is_deleted=0 AND status IN ('OPEN','IN_PROGRESS','PENDING_USER')")
    long countActive();

    @Select("SELECT COUNT(*) FROM nx_support_ticket WHERE is_deleted=0 AND status='PENDING_USER'")
    long countPendingUser();

    @Select("SELECT COUNT(*) FROM nx_support_ticket WHERE is_deleted=0 AND ops_unread_count>0 AND status<>'CLOSED'")
    long countOpsUnread();

    @Select("SELECT COUNT(*) FROM nx_support_ticket WHERE is_deleted=0 AND priority IN ('HIGH','URGENT') AND status IN ('OPEN','IN_PROGRESS','PENDING_USER')")
    long countHighPriorityActive();

    @Select("SELECT COUNT(*) FROM nx_support_ticket WHERE is_deleted=0 AND status IN ('RESOLVED','CLOSED')")
    long countArchived();
    ...
    @Select("""
            ...
            SELECT COUNT(*)
             ...
             <if test='(status == null or status == "") and scope == "archived"'>AND t.status IN ('RESOLVED','CLOSED')</if>
