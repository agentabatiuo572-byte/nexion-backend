...
public interface UserOpsMapper extends BaseMapper<UserEntity> {
    @Select("""
            SELECT *
             ...
             LIMIT 1 FOR UPDATE
            """)
    UserEntity lockActiveDevelopmentUserByPhone(
    ...
    @Update("""
            UPDATE nx_user_otp_challenge
             ...
             WHERE user_id=#{userId} AND challenge_no LIKE 'LOGIN-%'
               AND consumed_at IS NULL AND is_deleted=0
            """)
    int invalidateOpenLoginOtpChallenges(@Param("userId") Long userId);
     ...
     * 历史投影的 `member_user_id` 可能与页面展示的 `member_no` 不一致，因此规范 U 编号优先用于
     * 定位用户；非规范编号仍兼容 `member_user_id`。反规范化昵称只在用户行缺失或被软删时兜底。</p>
     */
    @Select("""
            SELECT COALESCE(u.id, tm.member_user_id) AS memberUserId,
              ...
              LEFT JOIN nx_user u
                ON u.id = CASE
                              WHEN tm.member_no REGEXP '^U[0-9]{8,}$'
                                  THEN CAST(SUBSTRING(tm.member_no, 2) AS UNSIGNED)
                              ELSE tm.member_user_id
                          END
               AND u.is_deleted = 0
