...
public class ComputeOrderPaidRocketListener {
    ...
    boolean consumeMessage(MessageExt rocketMessage) {
        ...
        if (!claim.claimed()) {
            log.info("Compute RocketMQ listener skipped duplicate eventId={}, status={}, attempts={}, msgId={}",
        ...
        if (!EVENT_ORDER_PAID.equals(message.getEventType())) {
            ...
            log.info("Compute RocketMQ listener skipped eventType={}, eventId={}, msgId={}",
        ...
        try {
            ...
            deliveryService.markSuccess(consumerGroup, claim.eventId(), activated);
            log.info("Compute RocketMQ listener activated orderNo={}, devices={}, eventId={}, msgId={}, reconsumeTimes={}",
                    payload.getOrderNo(), activated, claim.eventId(), rocketMessage.getMsgId(),
    ...
    private boolean recordMalformedFailure(MessageExt rocketMessage, Exception ex) {
        try {
            EventConsumerDeliveryService.ConsumerClaim claim = deliveryService.claim(
                    ...
                    rocketMessage.getReconsumeTimes());
            if (!claim.claimed()) {
                log.info("Compute RocketMQ listener skipped malformed duplicate eventId={}, status={}, msgId={}",
    ...
    private boolean markFailure(String eventId, MessageExt rocketMessage, Exception ex) {
        try {
            ...
            if (failure.dead()) {
                log.warn("Compute RocketMQ listener moved poison delivery to DEAD eventId={}, msgId={}, attempts={}, reconsumeTimes={}, error={}",
                        failure.eventId(), rocketMessage.getMsgId(), failure.attemptCount(),
                        rocketMessage.getReconsumeTimes(), errorMessage(ex));
                return false;
            }
            log.warn("Compute RocketMQ listener will retry eventId={}, msgId={}, attempts={}, reconsumeTimes={}, error={}",
                    failure.eventId(), rocketMessage.getMsgId(), failure.attemptCount(),
                    rocketMessage.getReconsumeTimes(), errorMessage(ex));
            return true;
        } catch (RuntimeException deliveryEx) {
    ...
    private boolean fallbackRetry(MessageExt rocketMessage, RuntimeException ex) {
        ...
        log.warn("Compute RocketMQ listener delivery-state error msgId={}, reconsumeTimes={}, retry={}, error={}",
