...
public class NotificationEarningGeneratedRocketListener {
    ...
    boolean consumeMessage(MessageExt rocketMessage) {
        ...
        if (!claim.claimed()) {
            log.info("Notification RocketMQ listener skipped duplicate eventId={}, status={}, attempts={}, msgId={}",
        ...
        if (!EVENT_EARNING_GENERATED.equals(message.getEventType())) {
            ...
            log.info("Notification RocketMQ listener skipped eventType={}, eventId={}, msgId={}",
        ...
        try {
            ...
            deliveryService.markSuccess(consumerGroup, claim.eventId(), notification == null ? 0 : 1);
            log.info("Notification RocketMQ listener created earning notification id={}, eventNo={}, eventId={}, msgId={}, reconsumeTimes={}",
                    notification == null ? null : notification.getId(), payload.getEventNo(), claim.eventId(),
    ...
    private boolean recordMalformedFailure(MessageExt rocketMessage, Exception ex) {
        try {
            ...
            if (!claim.claimed()) {
                log.info("Notification RocketMQ listener skipped malformed duplicate eventId={}, status={}, msgId={}",
    ...
    private boolean markFailure(String eventId, MessageExt rocketMessage, Exception ex) {
        try {
            ...
            if (failure.dead()) {
                log.warn("Notification RocketMQ listener moved poison delivery to DEAD eventId={}, msgId={}, attempts={}, reconsumeTimes={}, error={}",
                        failure.eventId(), rocketMessage.getMsgId(), failure.attemptCount(),
                        rocketMessage.getReconsumeTimes(), errorMessage(ex));
                return false;
            }
            log.warn("Notification RocketMQ listener will retry eventId={}, msgId={}, attempts={}, reconsumeTimes={}, error={}",
                    failure.eventId(), rocketMessage.getMsgId(), failure.attemptCount(),
                    rocketMessage.getReconsumeTimes(), errorMessage(ex));
            return true;
        } catch (RuntimeException deliveryEx) {
    ...
    private boolean fallbackRetry(MessageExt rocketMessage, RuntimeException ex) {
        ...
        log.warn("Notification RocketMQ listener delivery-state error msgId={}, reconsumeTimes={}, retry={}, error={}",
