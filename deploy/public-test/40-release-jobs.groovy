...
repositories.each { kind, repo ->
    def job = j.getItemByFullName("nexgrid-${kind}-test")
    ...
    if (!job.getTriggers().isEmpty()) throw new IllegalStateException('MANUAL_BUILD_ONLY_REQUIRED')
...
repositories.each { kind, repo ->
    ...
    job.setDescription('test CI; independent host broker verifies health/rollback before promotion. CI SUCCESS alone does not mean deployed.')
    job.save()
}
marker.setText('test release v1 configured; host auto deployment remains disabled until operator activation\n', 'UTF-8')
