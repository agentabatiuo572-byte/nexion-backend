...
VALUES
  ...
  (115, 'PERM_SYSTEM_WRITE', 'Write platform operations', 'API', '/api/admin/platform/**,/api/admin/commands/**', NULL, 1),
...
VALUES
  ...
  ('auth.risk.otp_ttl_minutes', '5', 'NUMBER', 'auth', 'PRIVATE', 'OTP lifetime in minutes.', 1),
...
VALUES
  ...
  ('high', '50 条', '每用户保留最新 50 条高优通知，超出部分淘汰最旧记录（按创建时间从早到晚）', 0, 20, 1, 'seed:i3', NOW(), NOW(), 0),
  ('normal', '200 条', '每用户保留最新 200 条常规通知，超出部分淘汰最旧记录（按创建时间从早到晚）', 0, 30, 1, 'seed:i3', NOW(), NOW(), 0),
